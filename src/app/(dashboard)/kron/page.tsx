...
import {
  ...
  RefreshCw, Clock, AlertTriangle, CheckCircle2, WifiOff,
  Plus, Settings
} from "lucide-react";
...
// ── Device Panel ──────────────────────────────────────────────────────────────
function DevicePanel({ device }: { device: KronDevice }) {
  const [latest, setLatest] = useState<KronReading | null>(
    device.readings[0] ?? null
  );
  ...
  const refresh = useCallback(async () => {
    setRefreshing(true);
    try {
      const r = await fetch(`/api/kron/devices/${device.id}/latest`);
    ...
    <div className="bg-slate-900/40 border border-slate-700/40 rounded-2xl p-5 space-y-4">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <div className={`w-2.5 h-2.5 rounded-full ${!latest ? 'bg-slate-600' : stale ? 'bg-yellow-500 animate-pulse' : 'bg-emerald-500 animate-pulse'}`} />
            <h2 className="text-white font-bold text-lg">{device.name}</h2>
          </div>
          <div className="flex flex-wrap gap-3 mt-1">
            {device.location && <span className="text-xs text-slate-500">📍 {device.location}</span>}
            {device.company && <span className="text-xs text-slate-500">🏢 {device.company.name}</span>}
            <span className="text-xs text-slate-600 font-mono">{device.mqttTopic}</span>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={refresh}
            disabled={refreshing}
            className="p-1.5 rounded-lg bg-slate-800/50 hover:bg-slate-700 text-slate-400 hover:text-white border border-slate-700 transition-all"
            title="Atualizar"
          >
      ...
      {!latest ? (
        <div className="flex flex-col items-center justify-center py-10 text-slate-500 gap-3">
          ...
          <p className="text-xs text-slate-600">Tópico: <code className="text-slate-500">{device.mqttTopic}</code></p>
        </div>
      ) : (
        <>
          {/* Alerta FP */}
          {(latest.powerFactor1 !== null && latest.powerFactor1 < 0.92) && (
            <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 text-sm">
              <AlertTriangle className="w-4 h-4 shrink-0" />
              <span>
                <strong>Fator de Potência baixo!</strong> Fase 1: {fmt(latest.powerFactor1, 3)}.
                Risco de multa de energia — verifique banco de capacitores.
              </span>
            </div>
          )}
...
export default function KronPage() {
  const [devices, setDevices] = useState<KronDevice[]>([]);
  ...
  useEffect(() => {
    async function load() {
      try {
        const [devRes, sessionRes] = await Promise.all([
          fetch('/api/kron/devices'),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            ...
            Medição Elétrica — KRON KS-3000
          </h1>
          <p className="text-slate-400 text-sm mt-1">
            Monitoramento em tempo real via MQTT · Atualização automática a cada 30s
          </p>
        </div>
        {isAdmin && (
          <a
            href="/admin/kron"
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-500/10 border border-violet-500/30 text-violet-300 hover:bg-violet-500/20 hover:text-violet-100 transition-all text-sm font-medium"
          >
      ...
      {!loading && devices.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-slate-500 gap-4 bg-slate-900/30 border border-slate-800 rounded-2xl">
          <Zap className="w-14 h-14 opacity-20" />
          <p className="text-lg font-medium text-slate-400">Nenhum medidor KRON cadastrado</p>
          <p className="text-sm text-slate-600 text-center max-w-sm">
            Acesse o painel de administração para adicionar o seu medidor KS-3000
            com o tópico MQTT configurado no software RedeMB TCP.
          </p>
          {isAdmin && (
            <a
              href="/admin/kron"
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/20 transition-all text-sm font-semibold"
            >
              <Plus className="w-4 h-4" />
              Adicionar Medidor KRON
            </a>
          )}
        </div>
      )}
