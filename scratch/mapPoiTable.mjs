...
session.walk('1.3.6.1.4.1.15921.60.2.10', 10, (snmpbinds) => {
  for (let i = 0; i < snmpbinds.length; i++) {
    ...
    // Mostrar todos os OIDs e seus valores para mapear quais são de Potência e Temperatura
    console.log(`${vb.oid} = [Type:${vb.type}] ${valStr}`);
  }
}, (error) => {
  if (error) {
